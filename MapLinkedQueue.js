 ...
 */
export default class MapLinkedQueue {
	#head = null;
	#items = new Map();
	...
	enque(...items) {
		...
		for (
			...
			item = this.#items.set(i, new QueueItem(items[i])).get(i);
			item.next = this.#items.get(i+1) || null;
		}
...
MapLinkedQueue.prototype.__proto__ = null; // chromium / v8 bug #5115
